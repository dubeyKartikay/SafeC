...
static void setCommitPtr(Segment *Seg, char *Ptr) {
  Seg->Other.CommitPtr = Ptr;
}
static void setReservePtr(Segment *Seg, char *Ptr) {
  Seg->Other.ReservePtr = Ptr;
}
...
static void setBigAlloc(Segment *Seg, int BigAlloc) {
  Seg->Other.BigAlloc = BigAlloc;
}
...
Object getObject(unsigned char *obj_ptr, Segment *seg) {
  ...
  if (seg->Other.BigAlloc) {
    ...
    if (PageNo >= 0 && PageNo < NUM_PAGES_IN_SEG && seg->Size[PageNo] == 1 &&
        Page + OBJ_HEADER_SIZE == (char *)obj_ptr) {
...
int isAddrOOB(void *base, void *accessed) {
  ...
  if (!seg) {
    printf("The base pointer is not a valid object isAddrOOB\n");
    return 1;
  }
...
int checkBounds(void *base, void *accessed, unsigned long long accessSize) {
  ...
  if (distance < 0 ||
      distance + accessSize - 1 >= obj.Header->Size - OBJ_HEADER_SIZE) {
...
int writeBarrierStack(void * base,void * dst ,void *src, ulong64 type) {
  unsigned long long distance = ((char *)dst) - (char *)base;
  printf("distance %d\n", distance);
  ulong64 bitm = 1 << (distance / 8);
  
  if (type & bitm) {
    printf("writeBarrier activated\n");
    if (src == NULL) {
      return 0;
    }
    ulong64 val = (ulong64)src;
    Segment *seg = getSegment(val);
    if (!seg) {
      printf("The  pointer is not a valid object\n");
      return 1;
    }
    Object obj = getObject((void *)val, seg);
    if (!obj.Header || obj.Header->Status == FREE) {
...
int writeBarrier(void *dst, void *src) {
  ulong64 val = (ulong64)dst;
  Segment *seg = getSegment(val);
  if (!seg) {
    printf("The base pointer is not a valid object\n");
    return 1;
  ...
  char *base_ptr = (char *)obj.data;
  unsigned long long distance = ((char *)dst) - base_ptr;
  printf("distance %d\n", distance);
  ulong64 bitm = 1 << (distance / 8);
  
  if (obj.Header->Type & bitm) {
    printf("writeBarrier activated\n");
    if (src == NULL) {
      return 0;
    }
...
int checkBoundsStack(void *base, void *accessed, unsigned long long accessSize,
                     unsigned long long allocSize) {
  unsigned long long distance = ((char *)accessed) - (char *)base;
  if (distance < 0 || distance + accessSize - 1 >= allocSize) {
    printf("Out of Bounds access\n");
    return 1;
  }
  printf("checkBoundsStack ok\n");
  return 0;
}
...
static Segment *allocateSegment(int BigAlloc) {
  void *Base =
      mmap(NULL, SEGMENT_SIZE * 2, PROT_NONE, MAP_ANON | MAP_PRIVATE, -1, 0);
  if (Base == MAP_FAILED) {
...
static ObjHeader *ObjToHeader(void *Obj) {
  return (ObjHeader *)((char *)Obj - OBJ_HEADER_SIZE);
}
...
int readArgv(const char *argv[], int idx) { return atoi(argv[idx]); }
